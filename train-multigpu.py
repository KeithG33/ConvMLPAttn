...
class Trainer:
    def __init__(
        ...
        if wandb_logging and self.accelerator.is_main_process:
        ...
        self.train_transform = transforms.Compose([
            ...
            transforms.RandomErasing(p=0.2, value='random', scale=(0.02, 0.1)),
        ...
        cosine_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer, T_max=decay_steps, eta_min=0.0005 * self.lr
        )
        ...
        if checkpoint_dir:
            ...
            if os.path.exists(metadata_path):
                with open(metadata_path, 'r') as f:
                    ...
                    self.accelerator.print(f"Loaded training state from {checkpoint_dir} (resuming from epoch {self.start_epoch})")
            else:
                self.accelerator.print(f"Loaded training state from {checkpoint_dir} (no metadata found, starting from epoch 0)")
    ...
    def run_epoch(self, epoch):
        ...
        pbar = tqdm(self.train_loader, desc=f"Epoch {epoch+1}/{self.epochs} - Training", leave=False, disable=not self.accelerator.is_local_main_process)
        for i, (images, labels) in enumerate(pbar):
            ...
            if i % self.log_every == 0:
                if self.wandb_logging and self.accelerator.is_main_process:
                    wandb.log({
    ...
    def run_validation(self):
        ...
        loss_sum = self.accelerator.gather_for_metrics(loss_sum).sum()
        correct1 = self.accelerator.gather_for_metrics(correct1).sum()
        correct5 = self.accelerator.gather_for_metrics(correct5).sum()
        count = self.accelerator.gather_for_metrics(count).sum()

        avg_loss = (loss_sum / count).item()
        top1 = (correct1 / count).item()
        top5 = (correct5 / count).item()
    ...
    def run_training(self):
        for epoch in tqdm(
            ...
            self.accelerator.print(f"Training - Epoch {epoch + 1}, Loss: {train_loss:.4f}")

            val_loss, val_acc, val_top5 = self.run_validation()
            self.accelerator.print(f"Validation - Acc@1: {val_acc:.4f}, Acc@5: {val_top5:.4f}, Loss: {val_loss:.4f}")
        ...
        self.accelerator.print(f"Training complete. Best validation metrics: Loss: {self.best_val_loss:.4f}, Acc: {self.best_val_acc:.4f}")
...
if __name__ == '__main__':
    trainer = Trainer(
        ...
        batch_size=768,
        lr=0.002,
        save_dir='./convmlpnet-hybrid2-spatialRPE-b1536-2gpu-lr0.002-decay0.0005',
        wandb_logging=True,  # set to True if you wish to log with wandb
        wandb_name='convmlpnet-hybrid2-spatialRPE-b1536-2gpu-lr0.002-decay0.0005', # wandb name
    )
